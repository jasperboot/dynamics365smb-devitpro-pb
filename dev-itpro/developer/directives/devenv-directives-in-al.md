...
The operators `AND`, `OR`, and `NOT` are supported in conditional directives. `AND` evaluates to `true` if both operands are true, `OR` evaluates to `true` if one of the operands is true, and `NOT` negates the value of the operand.
